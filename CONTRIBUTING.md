...
OpenTAMS is a cloud-agnostic implementation of the [BBC TAMS v8.0](https://bbc.github.io/tams/) media-asset API. This document is the working guide for the Amagi engineers who develop it: local setup, build and test commands, commit conventions, review, and releasing.

**Code contributions are accepted only from Amagi Media Labs Limited personnel.** We do not currently accept pull requests or code contributions from outside the organisation. Bug reports and feature requests are welcome from anyone through GitHub issues, and you are free to fork the repository under the [Apache 2.0 license](LICENSE).
...
make vuln-gate      # fails on reachable advisories not yet justified
make notices        # regenerate THIRD-PARTY-NOTICES from the module cache
make notices-check  # fails if THIRD-PARTY-NOTICES is stale
...
4. **The CI gate must be green** before review. The simplest way is `make ci` locally (build + vet + lint + race-checked tests). `make ci` includes `vuln-gate`, which fails on any advisory `govulncheck` can trace from OpenTAMS code that is not justified in [`.govulncheck-allow.yaml`](.govulncheck-allow.yaml). If a dependency upgrade is blocked, add an entry there saying why and what unblocks it — an entry without a reason is a hidden defect, not a suppression. Current status is in [docs/vulnerability-status.md](docs/vulnerability-status.md).
...
Third-party dependencies and their licences are listed in [THIRD-PARTY-NOTICES.txt](THIRD-PARTY-NOTICES.txt), which is generated by `make notices` — regenerate it after any dependency change, or CI fails. Attribution and trademark notices are in [NOTICE](NOTICE).
...
OpenTAMS is developed and maintained by [Amagi Media Labs Limited](https://www.amagi.com). The current roster is [MAINTAINERS.md](MAINTAINERS.md); how decisions get made is [GOVERNANCE.md](GOVERNANCE.md). Day-to-day:
